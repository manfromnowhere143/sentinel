...
| 23 | **S0-hardened causal localization** — same narrow motion/planning-bridge question, but artifact validity is the first research object | — (pre-registered; no run yet) | availability gate **PASS**: 66 eligible scenes, 39 fit / 13 calibration / 14 heldout, heldout keyframes **554** | **active pre-registration — no GPU extraction until canary/full extraction code is committed** | Iter23 keeps iteration-12 untouched and fixes the iter22 failure mode up front: local frame availability, split support, join key, tensor-shape stability, and canary determinism must pass before probes or interventions can exist. [`iter23_s0_hardened_causal_localization`](experiments/iter23_s0_hardened_causal_localization/HYPOTHESIS.md) |
...
- **Iteration 23 is pre-registered, not yet run.**
  ...
  next permitted action is to commit extraction patch, feeder, canary/full run scripts, analyzer,
  and canonical hashing routine. No GPU extraction, probe, activation direction, iteration-12
  scoring, or closed-loop work is authorized before that code is committed.
...
| [`experiments/iter23_s0_hardened_causal_localization/`](experiments/iter23_s0_hardened_causal_localization) | S0-hardened causal localization — active pre-registration; availability manifest committed; no model run yet |
