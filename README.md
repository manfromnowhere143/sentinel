...
| 28 | **official nuScenes trainval staging** — stage sensor blobs before a fresh atlas | — (pre-registered; no data movement yet) | target: official v1.0 trainval blob parts 1-10 only; destination `/datasets/nuscenes-full`; source manifest entries **10**; archive budget **292.78 GB**; availability bars reuse the staged-data discipline | **active pre-registration — staging only** | Daniel must perform any browser/session authentication or provide signed URLs; no credentials, model extraction, labels, probes, iter12, selector, or closed loop authorized. [`iter28_nuscenes_trainval_staging`](experiments/iter28_nuscenes_trainval_staging/HYPOTHESIS.md) |
...
- **Iteration 28 is pre-registered as official nuScenes trainval staging.**
  [`experiments/iter28_nuscenes_trainval_staging/HYPOTHESIS.md`](experiments/iter28_nuscenes_trainval_staging/HYPOTHESIS.md)
  authorizes only staging the ten official v1.0 trainval sensor blob archives into
  `/datasets/nuscenes-full`, then running a bounded availability inventory. It does not authorize
  model extraction, labels, probes, interventions, iteration-12, selector, or closed-loop work.
...
| [`experiments/iter28_nuscenes_trainval_staging/`](experiments/iter28_nuscenes_trainval_staging) | official nuScenes trainval staging — active pre-registration; no data movement yet |
