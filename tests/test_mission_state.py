        ...
        if relative == "MISSION_STATE.json":
            ...
            controller_source = (
                controller_source.replace(
                    launch_controller.GENERATION_SEVEN_RECEIPT_COMMIT,
                    generation_six["generation_six_receipt"],
                )
                .replace(
                    launch_controller.GENERATION_SEVEN_BATON_COMMIT,
                    generation_six_baton,
                )
                .replace('"generation": 8,', '"generation": 7,')
                .replace(
                    launch_controller.GENERATION_EIGHT_REASON,
                    GENERATION_SEVEN_REASON_CODE,
                )
            )
            path.write_text(controller_source)
