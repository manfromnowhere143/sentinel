...
def test_next_source_writer_detects_staging_replacement_and_preserves_foreign_name(
    ...
    temporary = tmp_path / f".next-source.{process_id:x}.{monotonic_tick:x}.tmp"
    output = tmp_path / "binding.json"
    replacement = tmp_path / "foreign-staging-source"
    ...
    def replacing_stat(
        ...
        *args: Any,
        ...
        nonlocal replaced
        if path == temporary.name and dir_fd is not None and not replaced:
    ...
    monkeypatch.setattr(verifier.os, "stat", replacing_stat)
...
@pytest.mark.parametrize("drift", ["payload", "mode", "link_count"])
...
def test_next_source_writer_final_name_replacement_is_committed(
    monkeypatch: Any, tmp_path: Path
) -> None:
    output = tmp_path / "binding.json"
    replacement = tmp_path / "replacement-binding"
    payload = b"committed-before-final-replacement\n"
    ...
    def tracking_open(
        ...
        nonlocal staging_descriptor
        descriptor = original_open(path, flags, mode, dir_fd=dir_fd)
        if isinstance(path, str) and path.startswith(".next-source.") and flags & os.O_EXCL:
            staging_descriptor = descriptor
        return descriptor

    def replace_then_read(*_args: Any) -> bytes:
        assert staging_descriptor is not None
        pinned = os.fstat(staging_descriptor)
        assert (pinned.st_dev, pinned.st_ino) != (
            replacement_identity.st_dev,
    ...
    monkeypatch.setattr(verifier.os, "open", tracking_open)
    monkeypatch.setattr(
        verifier, "_read_stable_regular_file_bounded", replace_then_read
    )
...
@pytest.mark.parametrize(
    ("publication_case", "expected_error"),
    [
        ("precommit", verifier.VerificationError),
        ("committed", verifier.NextSourceBindingCommittedError),
        ("indeterminate", verifier.NextSourceBindingIndeterminateError),
    ],
)
def test_next_source_writer_temporary_descriptor_close_fault_is_classified(
    monkeypatch: Any,
    tmp_path: Path,
    ...
    output = tmp_path / "binding.json"
    payload = b"descriptor-close-classification\n"
    original_close = os.close
    original_open = os.open
    original_stat = os.stat
    temporary_descriptor: int | None = None
    link_attempted = False
    reconciliation_faulted = False
    ...
    def faulting_close(descriptor: int) -> None:
        is_temporary = descriptor == temporary_descriptor
        original_close(descriptor)
        if is_temporary:
            raise OSError("temporary descriptor close fault")

    def failing_link(*_args: Any, **_kwargs: Any) -> None:
        nonlocal link_attempted
        link_attempted = True
        raise OSError("precommit link fault")

    def faulting_reconciliation_stat(
        path: Any,
        *args: Any,
        dir_fd: int | None = None,
        follow_symlinks: bool = True,
    ) -> os.stat_result:
        nonlocal reconciliation_faulted
        if (
            publication_case == "indeterminate"
            and link_attempted
            and not reconciliation_faulted
            and path == output.name
            and dir_fd is not None
        ):
            reconciliation_faulted = True
            raise OSError("link reconciliation fault")
        return original_stat(
            path, *args, dir_fd=dir_fd, follow_symlinks=follow_symlinks
        )
    ...
    monkeypatch.setattr(verifier.os, "open", tracking_open)
    monkeypatch.setattr(verifier.os, "close", faulting_close)
    if publication_case != "committed":
        monkeypatch.setattr(verifier.os, "link", failing_link)
    if publication_case == "indeterminate":
        monkeypatch.setattr(verifier.os, "stat", faulting_reconciliation_stat)

    with pytest.raises(expected_error, match="temporary descriptor close fail"):
        verifier._write_next_source_no_clobber(output, payload)  # noqa: SLF001

    if publication_case == "committed":
        assert output.read_bytes() == payload
        assert list(tmp_path.glob(".next-source.*.tmp")) == []
    elif publication_case == "precommit":
        assert not output.exists()
        assert list(tmp_path.glob(".next-source.*.tmp")) == []
    else:
        assert not output.exists()
        staging = list(tmp_path.glob(".next-source.*.tmp"))
