...
COMPLETE_VERDICT = "HUGSIM_TIMING_AWARE_BATCH_ACTOR_MATCH_AUDIT_COMPLETE"
SUPPORT_NULL_VERDICT = "HUGSIM_TIMING_AWARE_BATCH_ACTOR_MATCH_SUPPORT_NULL"
INFRA_NULL_VERDICT = "HUGSIM_TIMING_AWARE_BATCH_ACTOR_MATCH_INFRA_NULL"
MIN_CLASSIFIABLE = 4
...
def crosscheck_iter107(iter107_report: dict[str, Any], slots: list[dict[str, Any]], problems: list[str]) -> None:
    ...
    if not isinstance(summary, dict):
        ...
        summary = {}
    require_equal(problems, "iter107-completed-slot-count", summary.get("completed_slot_count"), 13)
    require_equal(problems, "iter107-artifact-count", summary.get("slot_proof_artifact_complete_count"), 13)
    require_equal(problems, "iter107-provenance-key-count", summary.get("collision_provenance_key_count"), 13)
    require_equal(problems, "iter107-slot-order", summary.get("collected_slot_ids_match_manifest"), True)
    require_equal(problems, "iter107-duplicate-groups", summary.get("duplicate_scenario_group_count"), 2)
    iter107_slots = iter107_report.get("slots")
    if not isinstance(iter107_slots, list):
        problems.append("iter107-slots-not-list")
        return
    manifest_ids = [slot.get("slot_id") for slot in slots]
    iter107_ids = [slot.get("slot_id") for slot in iter107_slots if isinstance(slot, dict)]
    require_equal(problems, "iter107-manifest-slot-ids", iter107_ids, manifest_ids)
...
def build_report(manifest_path: Path, iter107_report_path: Path, proof_root: Path) -> dict[str, Any]:
    ...
    infra_problems.extend(manifest_problems + iter107_problems)
    slots: list[dict[str, Any]] = []
    rows: list[dict[str, Any]] = []
    if not infra_problems:
        slots = load_manifest_slots(manifest, infra_problems)
        crosscheck_iter107(iter107_report, slots, infra_problems)
    ...
    summary = {
        ...
        "iter104_classifiable_baseline": ITER104_CLASSIFIABLE_BASELINE,
        "classifiable_delta_vs_iter104": classifiable - ITER104_CLASSIFIABLE_BASELINE,
    }
    return {
        "iteration": 108,
        "inputs": {
        ...
        "verdict": verdict,
        "claim_boundary": (
            "bounded 13-slot timing-aware actor-match support audit only; no repair, "
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    for row in report["episodes"]:
        lines.append(
            ...
            f"`{row.get('fire_timing_label')}` | `{row.get('first_fire_channel')}` | "
...
def run_analysis(
    ...
    report = build_report(manifest, iter107_report, proof_root)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--proof-root",
        type=Path,
        default=Path("experiments/iter107_hugsim_timing_aware_batch_execution/proof-execution"),
    )
    parser.add_argument(
        "--out",
        type=Path,
    ...
    parser.add_argument(
        ...
        default=Path(
            "experiments/iter108_hugsim_timing_aware_batch_actor_match_audit/proof-actor-match/"
            "timing_aware_batch_actor_match.md"
        ),
    )
