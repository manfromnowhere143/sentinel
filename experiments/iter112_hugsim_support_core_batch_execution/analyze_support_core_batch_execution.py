#!/usr/bin/env python3
"""Iteration 112 HUGSIM support-core batch execution proof analyzer."""
...
EXPECTED_MANIFEST_SHA = "a33888b174684b65758e74c949b05b695fda40ef3ddf0c7d9ee439a6ee818e99"
EXPECTED_STACK = {
...
DONE_MARKER = "I112_SUPPORT_CORE_BATCH_DONE"
...
def check_proof_sidecars(proof_root: Path, problems: list[str]) -> dict[str, Any]:
    sidecars = {
        ...
        "frozen_scenarios": proof_root / "frozen_scenarios_iter112.sha256",
        "run_log": proof_root / "i112-support-core-batch-run.log",
        "heavy_manifest": proof_root / "heavy_manifest_iter112.txt",
    }
...
def classify(summary: dict[str, Any], problems: list[str]) -> str:
    if problems:
        return "support_core_batch_execution_infra_null"
    complete = (
        ...
        and summary["duplicate_scenario_group_count"] == EXPECTED_DUPLICATE_GROUPS
    )
    return "support_core_batch_execution_complete" if complete else "support_core_batch_execution_infra_null"


def choose_verdict(label: str) -> str:
    if label == "support_core_batch_execution_complete":
        return "HUGSIM_SUPPORT_CORE_BATCH_EXECUTION_COMPLETE"
    return "HUGSIM_SUPPORT_CORE_BATCH_EXECUTION_INFRA_NULL"
...
def build_report(manifest_path: Path, proof_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "claim_boundary": (
            "HUGSIM support-core batch execution proof only; no actor-causality, actor-match "
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines.extend(
        [
            ...
            "## Slots",
            "",
            "| slot | slot id | scenario | run | role | complete | provenance rows | hdscore |",
            "|---:|---|---|---:|---|---|---:|---:|",
        ]
    )
    for row in report["slots"]:
        lines.append(
            ...
            f"`{row['run']}` | `{row.get('design_label')}` | `{row['complete']}` | "
            f"`{row.get('collision_provenance_rows')}` | `{row.get('hdscore')}` |"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=Path(
            "experiments/iter112_hugsim_support_core_batch_execution/proof-execution/"
            "support_core_batch_execution.md"
        ),
    )
