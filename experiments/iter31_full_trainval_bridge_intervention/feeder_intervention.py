#!/usr/bin/env python3
"""Row-manifest feeder for iteration-31 intervention replay.

Runs inside the UniAD model container. It serves only rows listed in the
committed iter31 replay manifest produced by build_direction.py.
"""
...
def load_row_manifest(path):
    ...
    for row in rows:
        ...
        if sample_index in grouped[scene]:
            raise SystemExit(f"duplicate manifest row {scene}[{sample_index}]")
...
def main():
    ...
    ap.add_argument("--row-manifest", default="/model/replay_manifest.json")
    ...
    ap.add_argument("--out", default="/model/sentinel_e31_gt.jsonl")
    args = ap.parse_args()

    grouped = load_row_manifest(args.row_manifest)
    ...
    for sc in ordered_scenes:
        ...
        for sample_index, smp in enumerate(chain):
            ...
            if timestamp_us != int(rec["timestamp_us"]):
                raise SystemExit(
            ...
            t0 = time.time()
            post(f"{base}/sentinel_e31_context", context)
        ...
        print(f"SCENE_DONE {sc['name']} frames={scene_frames}", flush=True)
    print(f"FEEDER_DONE alpha={args.alpha} scenes={len(ordered_scenes)} frames={done_frames}", flush=True)
