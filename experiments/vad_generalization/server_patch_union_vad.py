...
subprocess.run(['git', '-C', '/opt/sentinel-stack/VAD', 'checkout', '--', 'inference/server.py'], check=True)
src = open(SRV).read()
...
infer_old = ("    vad_output = vad_runner.forward_inference(vad_input)\n"
             "    return InferenceOutputs(\n"
             "        trajectory=vad_output.trajectory.tolist(),")
infer_new = ("    vad_output = vad_runner.forward_inference(vad_input)\n"
             "    _sentinel_log(vad_output, data.timestamp)\n"
             "    return InferenceOutputs(\n"
             "        trajectory=_sentinel_intervene(vad_output, data),")
