#!/bin/bash
# Iteration 47 Stage B — completion re-run of the 14 failed OFF-baseline episodes (box-side,
# detached per the playbook:
#   sudo bash -c 'setsid nohup bash /tmp/iter47_run_completion.sh </dev/null \
#     >/var/log/sentinel-iter47-completion.log 2>&1 &'
# This is the iteration-46 launcher adapted to the frozen iteration-47 schedule: ONLY the
# seven load_HD_map:true -medium-01 scenarios x 2 back-to-back runs. The 38 completed
# iteration-46 episodes are carried in place via resume-skip; the carried stochastic D0
# verdict is required (no re-probe); the Stage-A map jsons are a hard launch gate.
# Markers: I47_OFF_START, I47_OFF_PROVENANCE_OK|FAIL, I47_OFF_EP_START/RC/DONE pairs,
# I47_OFF_EP_SKIP_DONE, I47_OFF_PRIOR_LAUNCH_ARCHIVED, I47_OFF_REALCAR_COMPAT_LINKS,
# I47_OFF_PREP_FAIL, I47_OFF_ABORT_DISK, I47_OFF_ABORT_CONSECUTIVE_FAILURES,
# I47_OFF_COMPLETION_DONE. All frozen values mirror HYPOTHESIS.md (identical to iteration 46).
set -x
echo "I47_OFF_START $(date -u)"
...
# ---- frozen scenario manifest (all 52; provenance-verified even though only 14 run) ----
...
# ---- frozen iteration-47 schedule: the seven load_HD_map medium-01 scenarios, x2 each ----
SCHEDULE_SCENARIOS="scene-0038-medium-01
...
# The 38 carried episodes = (first 26 scenarios x r1,r2) minus the schedule above.
CARRIED_SCENARIOS="scene-0013-easy-00
scene-0013-medium-00
scene-0038-easy-00
scene-0038-medium-00
scene-0041-easy-00
scene-0041-medium-00
scene-0041-medium-01
scene-0051-easy-00
scene-0051-medium-00
scene-0062-easy-00
scene-0062-medium-00
scene-0064-easy-00
scene-0064-medium-00
scene-0071-easy-00
scene-0071-medium-00
scene-0138-easy-00
scene-0138-medium-00
scene-0166-easy-00
...
# Iteration-47 gate: the four Stage-A expansion vector maps must exist.
MAPS_OK=1
for loc in singapore-onenorth singapore-hollandvillage singapore-queenstown boston-seaport; do
    [ -s "$MAPS_EXPANSION/$loc.json" ] || MAPS_OK=0
done
if [ "$MAPS_OK" != "1" ]; then
    echo "I47_OFF_PROVENANCE_FAIL map expansion jsons missing (Stage A not staged)"
    PROV_FAIL=1
fi
# Iteration-47 gate: the carried D0 verdict is required — the branch decision was made
# once, in iteration 46, and is stochastic; no re-probe.
if [ "$(cat "$RUNS/d0_verdict.txt" 2>/dev/null)" != "stochastic" ]; then
    echo "I47_OFF_PROVENANCE_FAIL carried D0 verdict missing or not stochastic"
    PROV_FAIL=1
fi
# Iteration-47 gate: all 38 carried completed episodes present and non-failed.
CARRIED_OK=0
for s in $CARRIED_SCENARIOS; do
    for r in 1 2; do
        m="$RUNS/${s}__r${r}/episode_meta.json"
        if [ -f "$m" ] && ! grep -q '"failed":true' "$m"; then
            CARRIED_OK=$((CARRIED_OK + 1))
        else
            echo "I47_OFF_PROVENANCE_FAIL carried episode missing/failed: ${s}__r${r}"
            PROV_FAIL=1
        fi
    done
done
echo "I47_OFF_CARRIED_EPISODES present=$CARRIED_OK expected=38"
if [ "$(docker ps -q | wc -l)" != "0" ]; then
    echo "I47_OFF_PROVENANCE_FAIL another container is running; single-tenant rule"
    exit 1
fi
if [ "$PROV_FAIL" != "0" ]; then
    echo "I47_OFF_PROVENANCE_FAIL $(date -u)"
    exit 1
fi

# ---- archive prior-launch failure evidence (already committed in the iter46 proof) ----
shopt -s nullglob
...
if [ ${#STALE_FAILED[@]} -gt 0 ]; then
    ...
    echo "I47_OFF_PRIOR_LAUNCH_ARCHIVED $PRIOR failed_dirs=${#STALE_FAILED[@]}"
fi
shopt -u nullglob

{
    ...
    echo "  \"carried_d0_verdict\": \"$(cat "$RUNS/d0_verdict.txt")\","
    echo "  \"carried_completed_episodes\": $CARRIED_OK,"
    echo "  \"start_utc\": \"$(date -u +%Y-%m-%dT%H:%M:%SZ)\""
    echo "}"
} > "$RUNS/receipts_iter47.json"
echo "I47_OFF_PROVENANCE_OK $(date -u)"
...
echo "I47_OFF_REALCAR_COMPAT_LINKS linked=$LINKED"

disk_guard() {
    local avail
    avail=$(df --output=avail -BG /datasets/nuscenes-full | tail -1 | tr -dc '0-9')
    if [ "$avail" -lt "$DISK_MIN_GIB" ]; then
        echo "I47_OFF_ABORT_DISK avail=${avail}G $(date -u)"
        exit 1
    fi
}

prep_scene() {
    # iter46 amendment (a): extract via a temp dir keyed on cfg.yaml (7 of 19 zips nest
    # under a top-level 'nuscenes/' prefix); idempotent — all seven scenes are already
    # extracted from the completed iteration-46 episodes.
    local scene=$1
...
PYEOF
    ...
    if [ ! -f "$cfg" ]; then
        echo "I47_OFF_PREP_FAIL scene=$scene cfg.yaml missing after extraction"
        return 1
    fi
...
run_one() {
    ...
    if [ -f "$dest/episode_meta.json" ] && ! grep -q '"failed":true' "$dest/episode_meta.json"; then
        echo "I47_OFF_EP_SKIP_DONE $scenario r$run_idx (completed by a prior launch)"
        return 0
    fi
    prep_scene "$scene"
    ...
    for attempt in 1 2; do
        disk_guard
        echo "I47_OFF_EP_START $scenario r$run_idx a$attempt $(date -u)"
        local t0
        t0=$(date -u +%s)
        ...
        (cd "$HUGSIM" && timeout $EPISODE_TIMEOUT pixi run python -u closed_loop.py \
            ...
            --ad uniad --ad_cuda 0)
        rc=$?
        echo "I47_OFF_EP_RC=$rc $scenario r$run_idx a$attempt"
        docker rm -f hugsim_uniad_client >/dev/null 2>&1 || true
        hd=$(python3 -c "
...
")
        ...
        if [ "$rc" = "0" ] && [ "$hd" != "MISSING" ] && [ "$hd" != "INVALID" ] && [ "$steps" -gt 0 ]; then
            ...
            echo "I47_OFF_EP_DONE $scenario r$run_idx ok hd=$hd steps=$steps"
            return 0
        fi
        echo "I47_OFF_EP_ATTEMPT_FAIL $scenario r$run_idx a$attempt rc=$rc hd=$hd steps=$steps"
    done
    ...
    echo "I47_OFF_EP_DONE $scenario r$run_idx FAILED"
    return 1
}
...
note_result() {
    if [ "$1" = "0" ]; then
        ...
        CONSEC=$((CONSEC + 1))
        if [ "$CONSEC" -ge 3 ]; then
            echo "I47_OFF_ABORT_CONSECUTIVE_FAILURES $(date -u)"
            exit 1
        fi
    fi
}

# ---- frozen schedule: r1 then r2 back-to-back per scenario (within-launch pairing) ----
for s in $SCHEDULE_SCENARIOS; do
    run_one "$s" 1; note_result $?
    run_one "$s" 2; note_result $?
done

# ---- heavy-artifact manifest over all 52 episodes (iter46's heavy_manifest.txt untouched) ----
(cd "$RUNS" && find . -path ./prior_launches -prune -o -type f \( -name 'data.pkl' \
    -o -name 'infos.pkl' -o -name 'video.mp4' -o -name '*.ply' \) -print \
    | sort | xargs -r sha256sum > heavy_manifest_iter47.txt)
df -B1 /datasets/nuscenes-full | tail -1 >> "$RUNS/heavy_manifest_iter47.txt"
echo "I47_OFF_COMPLETION_DONE $(date -u)"
