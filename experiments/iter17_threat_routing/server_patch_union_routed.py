...
HELPERS = '''
# ---- Sentinel iter17: threat-class routing between the stop and the crawl -----------------------
# Identical triggers, thresholds, crawl re-parameterization (iter16) and K-frame release (iter15).
# ONE new mechanism: while latched, each frame routes by a path-overlap predicate — any tracked
# object whose constant-velocity path comes within SENTINEL_ROUTE_MARGIN of the ego's planned
# polyline mandates the committed STOP (position guarantee); if no object overlaps, the CRAWL
# keeps progress. The margin (2.0 m) is wider than the CPA margin so the router errs toward
# stopping.
...
def _sentinel_intervene(out, data):
    ...
    try:
        ...
        route_margin = float(_sos.environ.get("SENTINEL_ROUTE_MARGIN", "2.0"))
        dt = 0.5
        aux = out.aux_outputs.to_json()
        ...
        for i in range(min(len(objs), len(scores))):
            ...
            for k in range(H):
                ...
                if d < min_cpa:
                    min_cpa = d
                # path-overlap: object position at t vs the WHOLE planned polyline (spatial)
                for ex2, ey2 in ego_world_plan:
                    d2 = _smath.hypot(ex2 - ax, ey2 - ay)
                    if d2 < omin:
                        omin = d2
        ...
        if _sentinel_run.get("braking"):
            overlap = omin < route_margin
            act = "stop" if overlap else "crawl"
            try:
                with open(_SENTINEL_LOG, "a") as f:
                    f.write(_sjson.dumps({act: True, "run": _sentinel_run["i"],
                                          "cpa": round(min_cpa, 2), "ttc": round(min_ttc, 2),
                                          "omin": round(omin, 2),
            ...
            return _sentinel_crawl_traj(base, crawl_v, dt)
