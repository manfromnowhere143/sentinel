#!/usr/bin/env python3
"""Iteration-29 S0 integrity analysis."""
...
DTYPE_KEYS = ("sdc_traj_query_last_dtype", "sdc_track_query_dtype")
...
def analyze_pair(extract_path: str, gt_path: str) -> dict:
    ...
    for row in extract_nonreset:
        ...
        for dtype_key in DTYPE_KEYS:
            if dtype_key not in row:
                error_types[f"missing_{dtype_key}"] += 1
        if row.get("intervention_alpha") != 0.0:
            error_types["nonzero_intervention_alpha"] += 1
        if len(row.get("cands", [])) != 3:
            error_types["invalid_candidate_count"] += 1
    ...
    dtype_counts = {
        dtype_key: dict(Counter(row.get(dtype_key) for row in joined)) for dtype_key in DTYPE_KEYS
    }
    ...
    pass_s0 = (
        ...
        and all(len(counts) == 1 and None not in counts for counts in dtype_counts.values())
    )
    return {
