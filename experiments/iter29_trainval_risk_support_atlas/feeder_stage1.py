#!/usr/bin/env python3
"""Manifest-driven feeder for iteration-29 canary/full extraction.
...
def load_manifest(path, split_name, mode, canary_frames, canary_scenes_per_split):
    ...
    for split in order:
        ...
        if mode == "canary":
            records = records[:canary_scenes_per_split]
        for rec in records:
            rec = dict(rec)
            rec["split"] = split
            if mode == "canary":
...
def main():
    ...
    ap.add_argument("--canary-scenes-per-split", type=int, default=2)
    ...
    ap.add_argument("--out", default="/model/sentinel_e29_stage1_gt.jsonl")
    args = ap.parse_args()
    ...
    for sc in ordered_scenes:
        ...
        for sample_index, smp in enumerate(chain):
            ...
            t0 = time.time()
            post(f"{base}/sentinel_e29_context", context)
