#!/usr/bin/env python3
"""Iteration 107 HUGSIM timing-aware batch execution proof analyzer."""
...
EXPECTED_MANIFEST_SHA = "19d336364ab46f9e2e6bc881ffe4c7bad354471a851195b8609797d42e735f5a"
EXPECTED_STACK = {
...
DONE_MARKER = "I107_TIMING_AWARE_BATCH_DONE"
...
def check_proof_sidecars(proof_root: Path, problems: list[str]) -> dict[str, Any]:
    sidecars = {
        ...
        "frozen_scenarios": proof_root / "frozen_scenarios_iter107.sha256",
        "run_log": proof_root / "i107-timing-aware-batch-run.log",
        "heavy_manifest": proof_root / "heavy_manifest_iter107.txt",
    }
...
def classify(summary: dict[str, Any], problems: list[str]) -> str:
    if problems:
        return "timing_aware_batch_execution_infra_null"
    complete = (
        ...
        and summary["duplicate_scenario_group_count"] == 2
    )
    return "timing_aware_batch_execution_complete" if complete else "timing_aware_batch_execution_infra_null"


def choose_verdict(label: str) -> str:
    if label == "timing_aware_batch_execution_complete":
        return "HUGSIM_TIMING_AWARE_BATCH_EXECUTION_COMPLETE"
    return "HUGSIM_TIMING_AWARE_BATCH_EXECUTION_INFRA_NULL"
...
def build_report(manifest_path: Path, proof_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "claim_boundary": (
            "HUGSIM timing-aware batch execution proof only; no actor-causality, actor-match "
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=Path(
            "experiments/iter107_hugsim_timing_aware_batch_execution/proof-execution/"
            "timing_aware_batch_execution.md"
        ),
    )
