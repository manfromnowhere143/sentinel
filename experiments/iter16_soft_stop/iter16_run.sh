#!/bin/bash
# Iteration 16 — the crawl arm on all 20 scene-scenario pairs (120 episodes).
# OFF/union comparators are the committed full14 evidence; released is the committed iter15
# evidence (identical deterministic episodes).
exec > /var/log/sentinel-iter16.log 2>&1
set -x
...
python3 /tmp/server_patch_union_crawl.py || exit 1
...
for SCENARIO in stationary frontal side; do
  ...
  for seq in $SEQS; do
    echo "##### I16PAIR crawl $SCENARIO $seq #####"
