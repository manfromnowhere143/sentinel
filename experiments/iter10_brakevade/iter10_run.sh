#!/bin/bash
exec > /var/log/sentinel-i10.log 2>&1
set -x
...
RUNS=6
ARMS="off:0:0 union:1:0 brakevade:1:1"
PAIRS="stationary:0103 frontal:0103 side:0103"
for arm in $ARMS; do
  AN="${arm%%:*}"; r="${arm#*:}"; EN="${r%%:*}"; EV="${r##*:}"
  echo "##### I10_ARM_START $AN enabled=$EN evade=$EV $(date) #####"
  for pair in $PAIRS; do
    SCENARIO="${pair%%:*}"; seq="${pair##*:}"
    echo "##### I10PAIR $AN $SCENARIO $seq #####"
    ...
    BASE_DIR=$BASE_DIR NUSCENES_PATH=$NUSCENES_PATH MODEL_NAME=$MODEL_NAME MODEL_FOLDER=$MODEL_FOLDER \
     ...
     SENTINEL_TTC=2.5 SENTINEL_MIN_CLOSING=3 SENTINEL_EVADE=$EV SENTINEL_EVADE_LAT=3.0 SENTINEL_EVADE_DECEL=5.0 \
     SENTINEL_LOG=/model/sentinel_i10_$AN.jsonl \
     bash scripts/_docker_compose_release.sh $seq $SCENARIO --scenario-category=$SCENARIO --runs $RUNS
  done
  echo "##### I10_ARM_DONE $AN $(date) #####"
done
docker rm -f renderer model ncap >/dev/null 2>&1
echo "I10_ALL_DONE $(date)"
