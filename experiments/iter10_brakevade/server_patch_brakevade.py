...
HELPERS = '''
# ---- Sentinel iter10: side->stop ; frontal head-on -> BRAKING evasion into the tracked-CLEAR side ----
...
def _brake_evade(base, evade_dir, speed):
    # shed speed AND steer into the clear side: decelerate hard while ramping a moderate lateral offset
    lat_max = float(_sos.environ.get("SENTINEL_EVADE_LAT", "3.0"))
    decel = float(_sos.environ.get("SENTINEL_EVADE_DECEL", "5.0"))
    dt = 0.5
    pts = []
    dist = 0.0
    v = max(0.0, speed)
    for k in range(len(base)):
        ...
        dist += v * dt
        lat = evade_dir * lat_max * min(1.0, t / 1.0)
        pts.append([dist, lat])
    return pts
...
def _sentinel_intervene(out, data):
    ...
    try:
        ...
        cur_speed = _smath.hypot(base[0][0], base[0][1]) / dt if base else 8.0
        min_cpa = 1e9
        min_ttc = 1e9
        left_clear = 99.0   # nearest object on the ego's left within the forward zone
        right_clear = 99.0
        newtrack = {}
        for i in range(min(len(objs), len(scores))):
            ...
            if _smath.hypot(ox, oy) > max_gap:
                continue
            # clear-side bookkeeping: objects roughly ahead, split by lateral side
            if -5.0 < ox < 30.0:
                if oy > 0.5:
        ...
        if _sentinel_run.get("mode") is None:
            if evade_on and min_ttc < ttc_thresh:
                _sentinel_run["mode"] = "brakevade"
                _sentinel_run["evade_dir"] = 1.0 if left_clear >= right_clear else -1.0  # steer into clearer side
        ...
        if mode:
            try:
                ...
                pass
            if mode == "brakevade":
                return _brake_evade(base, _sentinel_run["evade_dir"], _sentinel_run["spd"])
