...
HELPERS = '''
# ---- Sentinel: shadow log + brake from PLAN-vs-TRACKED-PATH closest approach (iter6) -----------
...
def _sentinel_intervene(out, data):
    """Brake when the ego's PLANNED path and an agent's TRACKED path reach a collision-close point
    at the same time (closest point of approach over both real trajectories, world frame)."""
    base = [[float(x), float(y)] for x, y in out.trajectory.tolist()]
    if _sos.environ.get("SENTINEL_ENABLED", "0") != "1":
        return base
    try:
        cpa_margin = float(_sos.environ.get("SENTINEL_CPA_MARGIN", "2.5"))
        ...
        # ego PLANNED path in world coordinates (where the planner intends to be at each step)
        ego_world_plan = []
        for px, py in base:
        ...
        for i in range(min(len(objs), len(scores))):
            ...
            # agent world velocity from tracking; 0 (static) until a second sighting
            avx = avy = 0.0
            ...
            # closest approach between the ego's planned path and the agent's extrapolated path
            for k in range(H):
                t = (k + 1) * dt
        ...
        if _sentinel_run.get("braking") or min_cpa < cpa_margin:
