#!/bin/bash
exec > /var/log/sentinel-i6.log 2>&1
set -x
...
for arm in $ARMS; do
  AN="${arm%%:*}"; EN="${arm##*:}"
  echo "##### I6_ARM_START $AN enabled=$EN $(date) #####"
  for pair in $PAIRS; do
    SCENARIO="${pair%%:*}"; seq="${pair##*:}"
    echo "##### I6PAIR $AN $SCENARIO $seq #####"
    ...
    BASE_DIR=$BASE_DIR NUSCENES_PATH=$NUSCENES_PATH MODEL_NAME=$MODEL_NAME MODEL_FOLDER=$MODEL_FOLDER \
     ...
     SENTINEL_ENABLED=$EN SENTINEL_MIN_SCORE=0.3 SENTINEL_MAXGAP=30 SENTINEL_CPA_MARGIN=2.5 \
     SENTINEL_LOG=/model/sentinel_i6_$AN.jsonl \
     bash scripts/_docker_compose_release.sh $seq $SCENARIO --scenario-category=$SCENARIO --runs $RUNS
  done
  echo "##### I6_ARM_DONE $AN $(date) #####"
done
docker rm -f renderer model ncap >/dev/null 2>&1
echo "I6_ALL_DONE $(date)"
