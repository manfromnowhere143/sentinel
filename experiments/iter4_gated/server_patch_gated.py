...
HELPERS = '''
# ---- Sentinel: shadow logging + gated-TTC emergency-brake intervention ----------------------
...
def _sentinel_intervene(out):
    """Brake (committed stop) when an agent ACTIVELY closing on the ego has low time-to-collision."""
    ...
    try:
        ...
        min_close = float(_sos.environ.get("SENTINEL_MIN_CLOSING", "0.0"))  # 0 = total-closing (old); >0 = agent-only gate
        dt = 0.5
        aux = out.aux_outputs.to_json()
        ...
        for i in range(min(len(objs), len(scores), len(futs))):
            ...
            if nm:
                avx /= nm; avy /= nm
            if min_close > 0.0:
                # gated: only the AGENT's own approach toward the ego counts (ignore ego approaching passive objects)
                closing = -(avx * ox + avy * oy) / gap
                gate = max(min_close, 0.5)
            else:
                # old: total closing (ego approach + agent approach)
