...
Frozen after iteration 103 was published, but before any iteration-104 analyzer, actor-match
classification, proof artifact, result, or claim. This is an offline analyzer over the committed
iteration-103 proof only. It launches no GPU work and changes no code under test.

## Process disclosure

This is not blind. Iteration 103 has already published the execution proof:

- `13/13` registered slots completed;
- `0` retries;
- `13/13` proof artifact sets complete;
- `13/13` evals expose the top-level `collision_provenance` key;
- `217` total collision-provenance rows;
- `9` unique scenarios and all `4` duplicate scenario groups preserved by `slot_id`.

Those are execution-support facts only. No actor-match classifier has been run over the iteration
103 proof before this file. The bars below freeze how support and match labels will be computed.

## Research question

Using the frozen iteration-59 actor-match support rules, does the 13-slot iteration-103 proof
support same-run comparison between:
...
and if support exists, how many classifiable rows are actor matches, mismatches, or ambiguous?

This iteration may answer only support and descriptive match classification inside the registered
13-slot proof. It does not claim the distribution of all HUGSIM failures.

## Frozen inputs

- Iteration 102 manifest:
  `experiments/iter102_hugsim_provenance_batch_launch_manifest/proof-launch-manifest/provenance_batch_launch_manifest.json`
- Iteration 103 execution report:
  `experiments/iter103_hugsim_provenance_batch_execution/proof-execution/provenance_batch_execution_report.json`
- Iteration 103 proof root:
...
iteration-103 proof root. It may not read live GPU state, raw box paths outside committed proof,
...
- `HUGSIM_PROVENANCE_BATCH_ACTOR_MATCH_INFRA_NULL`: iteration 103 is not
  `HUGSIM_PROVENANCE_BATCH_EXECUTION_COMPLETE`; the iteration-102 manifest and iteration-103
...
- `HUGSIM_PROVENANCE_BATCH_ACTOR_MATCH_SUPPORT_NULL`: infrastructure passes, but fewer than `4`
  slots are `classifiable_foreground`.
- `HUGSIM_PROVENANCE_BATCH_ACTOR_MATCH_AUDIT_COMPLETE`: infrastructure passes and at least `4`
  slots are `classifiable_foreground`. Match/mismatch/ambiguous counts are descriptive outputs
  under this bounded support audit.

## Required proof artifacts

- analyzer source plus unit tests;
- `proof-actor-match/provenance_batch_actor_match_report.json`;
- `proof-actor-match/provenance_batch_actor_match.md`;
- `proof-actor-match/analyze_provenance_batch_actor_match.command.txt`.
...
3. Run the analyzer once over committed iteration-103 proof.
4. Publish `RESULT.md`, update docs/handoff, run `ruff check .`, `pytest -q`, and
   `python3 scripts/validate_docs.py`, then push.

## Forbidden claims

No repair, threshold-value, transfer, safety, deployment, robustness, benchmark,
population-rate, HD-Score-invariance, real-world behavior, first-responder behavior,
acquisition-value, retuning, or production claim. A match count is not proof that Sentinel
prevents collisions; a mismatch count is not proof of a repair. This iteration may only claim
whether the registered iteration-103 slots support same-run monitor-hazard versus HUGSIM
collision-actor comparison and report descriptive labels if support exists.
