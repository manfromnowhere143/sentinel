...
# Box-side launch:
#   sudo bash -c 'setsid nohup bash /tmp/iter103_run_provenance_batch_execution.sh </dev/null \
#     >/var/log/sentinel-iter103-provenance-batch.log 2>&1 &'
# Requires:
#   /tmp/iter103_hugsim_provenance.patch
#   /tmp/iter103_client_patch.py
#   /tmp/iter103_provenance_batch_launch_manifest.json
set -x
echo "I103_START $(date -u)"
...
RUNS=/datasets/nuscenes-full/hugsim/iter103_runs
...
HUGSIM_PATCH=/tmp/iter103_hugsim_provenance.patch
MONITOR_PATCH=/tmp/iter103_client_patch.py
MANIFEST=/tmp/iter103_provenance_batch_launch_manifest.json
SHIM=/opt/sentinel-stack/hugsim-shim/sitecustomize.py
CKPT=/opt/sentinel-stack/UniAD/ckpts/uniad_base_e2e.pth

FROZEN_MANIFEST_SHA=ddbc9960fe0b50b95842bd2c8b2e26b5e7ed00abba65f8f97b5ed6515c428870
...
fail_gate() {
    echo "I103_PROVENANCE_BATCH_FAIL $1 $(date -u)"
    exit 1
}
...
disk_guard() {
    ...
    if [ "$avail" -lt "$DISK_MIN_GIB" ]; then
        echo "I103_ABORT_DISK avail=${avail}G $(date -u)"
        exit 1
    fi
}
...
PYEOF
    ...
    if [ ! -f "$cfg" ]; then
        echo "I103_PREP_FAIL scene=$scene cfg.yaml missing after extraction"
        return 1
    fi
...
apply_hugsim_patch() {
    ...
    if git -C "$HUGSIM" apply --check "$HUGSIM_PATCH"; then
        ...
        echo "I103_HUGSIM_PATCH_APPLIED"
    elif git -C "$HUGSIM" apply --reverse --check "$HUGSIM_PATCH"; then
        echo "I103_HUGSIM_PATCH_ALREADY_APPLIED"
    else
...
write_manifest_files() {
    ...
    echo "$FROZEN_MANIFEST_SHA  iter103_provenance_batch_launch_manifest.json" \
        > "$RUNS/frozen_manifest.sha256"
    python3 - "$MANIFEST" "$RUNS/frozen_scenarios_iter103.sha256" "$RUNS/slot_schedule_iter103.tsv" <<'PYEOF'
...
run_one() {
    ...
    if [ -f "$dest/episode_meta.json" ] && ! grep -q '"failed":true' "$dest/episode_meta.json"; then
        echo "I103_SLOT_SKIP_DONE $slot_id $scenario on"
        return 0
    fi
    ...
    for attempt in 1 2; do
        disk_guard
        echo "I103_SLOT_START $slot_id $scenario on a$attempt $(date -u)"
        local t0
        t0=$(date -u +%s)
        ...
        (cd "$HUGSIM" && SENTINEL_ENABLED=1 timeout "$EPISODE_TIMEOUT" pixi run python -u closed_loop.py \
            ...
            --ad uniad --ad_cuda 0)
        rc=$?
        echo "I103_SLOT_RC=$rc $slot_id $scenario on a$attempt"
        docker rm -f hugsim_uniad_client >/dev/null 2>&1 || true
        hd=$(python3 -c "
import json, math
try:
...
")
        ...
        if [ "$rc" = "0" ] && [ "$hd" != "MISSING" ] && [ "$hd" != "INVALID" ] \
            ...
            echo "I103_SLOT_DONE $slot_id $scenario on ok hd=$hd steps=$steps"
            return 0
        fi
        echo "I103_SLOT_ATTEMPT_FAIL $slot_id $scenario on a$attempt rc=$rc hd=$hd provenance=$provenance steps=$steps marker=$marker decisions=$decisions"
    done
    rm -rf "${dest}__failed"
    ...
    echo "I103_SLOT_DONE $slot_id $scenario on FAILED"
    return 1
}
...
write_manifest_files
(cd "$SCEN_DIR" && sha256sum -c "$RUNS/frozen_scenarios_iter103.sha256") || fail_gate "scenario SHA mismatch"
...
echo "I103_REALCAR_COMPAT_LINKS linked=$LINKED"

{
    ...
    echo "  \"schedule\": \"thirteen ON slots from Iter102 manifest\","
    echo "  \"slot_count\": 13,"
    echo "  \"start_utc\": \"$(date -u +%Y-%m-%dT%H:%M:%SZ)\""
    echo "}"
} > "$RUNS/receipts.json"
echo "I103_PROVENANCE_OK $(date -u)"

while IFS=$'\t' read -r slot_id scenario _scenario_sha; do
    run_one "$slot_id" "$scenario" || exit 1
done < "$RUNS/slot_schedule_iter103.tsv"

(cd "$RUNS" && find . -path ./prior_launches -prune -o -type f \( -name 'data.pkl' \
    -o -name 'infos.pkl' -o -name 'video.mp4' -o -name '*.ply' \) -print \
    | sort | xargs -r sha256sum > heavy_manifest_iter103.txt)
df -B1 /datasets/nuscenes-full | tail -1 >> "$RUNS/heavy_manifest_iter103.txt"
echo "I103_PROVENANCE_BATCH_DONE $(date -u)"
