#!/bin/bash
exec > /var/log/sentinel-i7.log 2>&1
set -x
...
RUNS=6
# arm = name:enabled:cpa_margin
ARMS="off:0:2.5 cpa15:1:1.5 cpa10:1:1.0"
PAIRS="stationary:0103 frontal:0103 side:0103"
for arm in $ARMS; do
  AN="${arm%%:*}"; r="${arm#*:}"; EN="${r%%:*}"; MG="${r##*:}"
  echo "##### I7_ARM_START $AN enabled=$EN cpa_margin=$MG $(date) #####"
  for pair in $PAIRS; do
    SCENARIO="${pair%%:*}"; seq="${pair##*:}"
    echo "##### I7PAIR $AN $SCENARIO $seq #####"
    ...
    BASE_DIR=$BASE_DIR NUSCENES_PATH=$NUSCENES_PATH MODEL_NAME=$MODEL_NAME MODEL_FOLDER=$MODEL_FOLDER \
     ...
     SENTINEL_ENABLED=$EN SENTINEL_MIN_SCORE=0.3 SENTINEL_MAXGAP=30 SENTINEL_CPA_MARGIN=$MG \
     SENTINEL_LOG=/model/sentinel_i7_$AN.jsonl \
     bash scripts/_docker_compose_release.sh $seq $SCENARIO --scenario-category=$SCENARIO --runs $RUNS
  done
  echo "##### I7_ARM_DONE $AN $(date) #####"
done
docker rm -f renderer model ncap >/dev/null 2>&1
echo "I7_ALL_DONE $(date)"
