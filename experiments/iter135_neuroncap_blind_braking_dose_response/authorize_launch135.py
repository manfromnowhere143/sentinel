...
GIT_FAILURE_DIAGNOSTIC_RAW_BYTE_LIMIT = 4 * 1024
GIT_FAILURE_DIAGNOSTIC_PREVIEW_BYTE_LIMIT = 2 * 1024
GIT_FAILURE_DIAGNOSTIC_TRUNCATION_MARKER = "...<diagnostic truncated>"


def _git_failure_preview(payload: bytes) -> tuple[str, bool]:
    retained = payload[:GIT_FAILURE_DIAGNOSTIC_RAW_BYTE_LIMIT]
    escaped = json.dumps(
        retained.decode("utf-8", errors="backslashreplace"),
        ensure_ascii=True,
    ).encode("ascii")
    truncated = len(retained) != len(payload) or len(escaped) > (
        GIT_FAILURE_DIAGNOSTIC_PREVIEW_BYTE_LIMIT
    )
    if not truncated:
        return escaped.decode("ascii"), False
    retained_byte_count = (
        GIT_FAILURE_DIAGNOSTIC_PREVIEW_BYTE_LIMIT
        - len(GIT_FAILURE_DIAGNOSTIC_TRUNCATION_MARKER.encode("ascii"))
    )
    return (
        escaped[:retained_byte_count].decode("ascii")
        + GIT_FAILURE_DIAGNOSTIC_TRUNCATION_MARKER,
        True,
    )
...
def _git_failure_diagnostic(
    ...
    *,
    ...
    if not include_preview:
        return diagnostic
    preview, truncated = _git_failure_preview(payload)
    return (
        f"{diagnostic} "
        f"{label}_preview_truncated={str(truncated).lower()} "
        f"{label}_preview={preview}"
    )
...
def _git(repo: Path, *arguments: str, timeout: int = 10) -> bytes:
    ...
    if completed.returncode != 0:
        ...
        raise AuthorizationError(
            f"Git probe failed: {arguments[0]} "
            f"return_code={completed.returncode} "
            f"{_git_failure_diagnostic('stdout', stdout)} "
            f"{_git_failure_diagnostic('stderr', stderr, include_preview=True)}"
        )
