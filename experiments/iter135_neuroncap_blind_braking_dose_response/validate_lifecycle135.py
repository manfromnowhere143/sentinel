...
SCHEMA = "iter135.lifecycle_observation.v3"
SOURCE_SCHEMA = "iter135.lifecycle_source_observation.v3"
...
QUERY_PARSERS = {
    ...
    "global_containers": "bounded-unfiltered-global-container-records.v3",
...
ENUMERATION_SCOPES = {
    ...
    "global_containers": "complete-host-runtime-container-registry",
    ...
    *,
    ...
    if source == "global_containers":
        ...
        for index, record in enumerate(records):
            ...
            problems.extend(_field_set(record, RECORD_FIELDS[source], record_label))
            if type(record) is not dict:
                continue
            container_id = record.get("container_id")
            if type(container_id) is not str or _RESOURCE_ID.fullmatch(container_id) is None:
                problems.append(f"{record_label}:container-id")
            if record.get("runtime_state") != "RUNNING":
                problems.append(f"{record_label}:runtime-state")
            for field in ("iteration_label", "attempt_label", "manifest_label"):
                ...
                if value is not None and (
                    ...
                    problems.append(f"{record_label}:{field.replace('_', '-')}")
        if problems:
            return sorted(set(problems)), None
        return [], SourceFact(
            "HOST_OCCUPIED",
    ...
    elif source == "mission_containers":
        container_id = record.get("container_id")
        if type(container_id) is not str or _RESOURCE_ID.fullmatch(container_id) is None:
...
def _observations(
    ...
    list[str],
    dict[str, tuple[SourceFact | None, SourceFact | None]],
    dict[
        str,
        tuple[
    ...
    for source in SOURCE_NAMES:
        ...
        parsed_records: list[tuple[Mapping[str, Any], ...] | None] = []
        for position in ("initial", "terminal"):
            ...
            parsed_records.append(
                tuple(observation["records"]) if observation is not None else None
            )
            if observation is not None:
                successful_digests[position].append(capture["stdout_sha256"])
        facts[source] = (parsed[0], parsed[1])
        records_by_source[source] = (parsed_records[0], parsed_records[1])
    ...
    return sorted(set(problems)), facts, records_by_source


def _container_inventory_relationship(
    records_by_source: Mapping[
        str,
        tuple[
            tuple[Mapping[str, Any], ...] | None,
            tuple[Mapping[str, Any], ...] | None,
    ...
    """Return exact mission/global consistency and stable foreign-host occupancy."""

    foreign_container_present = False
    for round_index in (0, 1):
        mission_records = records_by_source["mission_containers"][round_index]
        global_records = records_by_source["global_containers"][round_index]
        if mission_records is None or global_records is None:
            return False, False
        expected_global_records = {
            _canonical_json(
                {
                    "attempt_label": record["attempt_id"],
                    "container_id": record["container_id"],
                    "iteration_label": str(ITERATION),
                    "manifest_label": record["manifest_sha256"],
                    "runtime_state": record["runtime_state"],
                }
            )
            for record in mission_records
        }
        global_record_bytes = {_canonical_json(record) for record in global_records}
        mission_labelled_global_records = {
            _canonical_json(record)
            for record in global_records
            if record["iteration_label"] == str(ITERATION)
        }
        if expected_global_records != mission_labelled_global_records:
            return False, False
        foreign_container_present |= bool(global_record_bytes - expected_global_records)
    return True, foreign_container_present
...
def _reduce(receipt: Mapping[str, Any]) -> tuple[str, list[str]]:
    ...
    observation_problems, facts_by_source, records_by_source = _observations(receipt)
    ...
    if not available:
        return "UNKNOWN", []
    global_initial, global_terminal = facts_by_source["global_containers"]
    if (
        global_initial is not None
        and global_terminal is not None
        and global_initial != global_terminal
    ):
        return "UNKNOWN", []
    container_inventories_consistent, foreign_container_present = _container_inventory_relationship(
        records_by_source
    )
    if not container_inventories_consistent:
        return "UNKNOWN", []
    if any(
        initial is not None and terminal is not None and initial != terminal
        for source, (initial, terminal) in facts_by_source.items()
        if source != "global_containers"
    ):
        return "INCONSISTENT", []
    stable = [
    ...
    positive_bindings = {
        (fact.attempt_id, fact.manifest_sha256) for fact in [*active, *terminal]
    }
    if positive_bindings:
        if len(positive_bindings) != 1:
            return "INCONSISTENT", []
        attempt_id, manifest_sha256 = next(iter(positive_bindings))
        if (
            mission.get("attempt_id") != attempt_id
            or mission.get("manifest_sha256") != manifest_sha256
        ):
            return "INCONSISTENT", []
    terminal_binding: tuple[str, str | None, str | None, str | None, str | None] | None = None
    if terminal:
        terminal_bindings = {
            (
                fact.kind,
        ...
        if len(terminal_bindings) != 1:
            return "INCONSISTENT", []
        terminal_binding = next(iter(terminal_bindings))
    if foreign_container_present:
    ...
    if terminal:
        ...
        terminal_state = terminal_binding[0]
        if any(
            pair[0] is not None
    ...
    if all(fact.kind == "EMPTY" for fact in stable):
        if mission.get("attempt_id") is not None or mission.get("manifest_sha256") is not None:
            return "INCONSISTENT", []
        global_records = records_by_source["global_containers"]
        if global_records[0] != () or global_records[1] != ():
            return "UNKNOWN", []
