#!/bin/bash
exec > /var/log/sentinel-i5.log 2>&1
set -x
...
RUNS=6
# arm = name:enabled:minclosing:trackvel
ARMS="off:0:0:0 tracked:1:3:1"
PAIRS="stationary:0103 frontal:0103 side:0103"
for arm in $ARMS; do
  AN="${arm%%:*}"; r="${arm#*:}"; EN="${r%%:*}"; r="${r#*:}"; MC="${r%%:*}"; TV="${r##*:}"
  echo "##### I5_ARM_START $AN enabled=$EN min_closing=$MC track_vel=$TV $(date) #####"
  for pair in $PAIRS; do
    SCENARIO="${pair%%:*}"; seq="${pair##*:}"
    echo "##### I5PAIR $AN $SCENARIO $seq #####"
    ...
    BASE_DIR=$BASE_DIR NUSCENES_PATH=$NUSCENES_PATH MODEL_NAME=$MODEL_NAME MODEL_FOLDER=$MODEL_FOLDER \
     ...
     SENTINEL_ENABLED=$EN SENTINEL_TTC=2.5 SENTINEL_MIN_SCORE=0.3 SENTINEL_MIN_CLOSING=$MC \
     SENTINEL_MAXGAP=25 SENTINEL_TRACK_VEL=$TV SENTINEL_LOG=/model/sentinel_i5_$AN.jsonl \
     bash scripts/_docker_compose_release.sh $seq $SCENARIO --scenario-category=$SCENARIO --runs $RUNS
  done
  echo "##### I5_ARM_DONE $AN $(date) #####"
done
docker rm -f renderer model ncap >/dev/null 2>&1
echo "I5_ALL_DONE $(date)"
