#!/usr/bin/env python3
"""Build the iter24 availability and split manifest.
...
EXPERIMENT = "iter24_risk_support_atlas"
...
def split_hash(scene_name: str) -> str:
    return hashlib.sha256(f"iter24:{scene_name}".encode()).hexdigest()
...
def main() -> int:
    ...
    parser.add_argument(
        "--out-dir",
        default="experiments/iter24_risk_support_atlas",
    ...
    known_data_scene_names = set(known_data_firewall["scene_names"])
    excluded = sorted(
        set(NEURO_NCAP_OFFICIAL_SCENES)
        | set(ITER12_EVALUATION_SCENES)
        | known_data_scene_names
    )
    ...
    total_keyframes = sum(int(r["available_keyframes"]) for r in eligible_sorted)
    heldout_keyframes = sum(int(r["available_keyframes"]) for r in splits["heldout"])
    availability_pass = (
        len(eligible_sorted) >= MIN_ELIGIBLE_SCENES
    ...
    manifest = {
        ...
        "hypothesis": "experiments/iter24_risk_support_atlas/HYPOTHESIS.md",
        ...
        "selection_rule": (
            "Exclude NeuroNCAP/iteration-12 and iter22/iter23 known-data scene names; require "
            "local keyframe availability; sort eligible scenes by SHA256('iter24:<scene_name>'); "
            "assign first 50% fit, next 25% calibration, remainder heldout."
        ),
        "counts": {
    ...
    report = [
        "iter24 risk-support availability manifest report",
        ...
        f"known_data_scene_names: {' '.join(known_data_firewall['scene_names'])}",
        ...
        f"total_keyframe_count: {total_keyframes}",
