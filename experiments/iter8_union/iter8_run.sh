#!/bin/bash
exec > /var/log/sentinel-i8.log 2>&1
set -x
...
for arm in $ARMS; do
  AN="${arm%%:*}"; EN="${arm##*:}"
  echo "##### I8_ARM_START $AN enabled=$EN $(date) #####"
  for pair in $PAIRS; do
    SCENARIO="${pair%%:*}"; seq="${pair##*:}"
    echo "##### I8PAIR $AN $SCENARIO $seq #####"
    ...
    BASE_DIR=$BASE_DIR NUSCENES_PATH=$NUSCENES_PATH MODEL_NAME=$MODEL_NAME MODEL_FOLDER=$MODEL_FOLDER \
     ...
     SENTINEL_TTC=2.5 SENTINEL_MIN_CLOSING=3 SENTINEL_LOG=/model/sentinel_i8_$AN.jsonl \
     bash scripts/_docker_compose_release.sh $seq $SCENARIO --scenario-category=$SCENARIO --runs $RUNS
  done
  echo "##### I8_ARM_DONE $AN $(date) #####"
done
docker rm -f renderer model ncap >/dev/null 2>&1
echo "I8_ALL_DONE $(date)"
