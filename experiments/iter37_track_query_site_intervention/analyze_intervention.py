#!/usr/bin/env python3
"""Analyze iteration-37 prefix-preserving track-query intervention replay logs.
...
def analyze_heldout(paths: list[Path]) -> dict:
    ...
    if prefix_failures:
        ...
        verdict = "TRACK_QUERY_INTERVENTION_STAGE1_PASS_SUCCESSOR_PREREG_AUTHORIZED"
    return {
        "stage": "iter37_heldout_track_query_intervention",
        "command_line": " ".join(sys.argv),
        "run_summaries": [public_run_summary(summary) for summary in summaries],
        "prefix_replay_integrity_pass": not prefix_failures,
        "prefix_replay_integrity_failures": prefix_failures[:100],
    ...
    return {
        ...
        **site_report,
        ...
        "claim_boundary": (
            ...
            "compatible target-row projection so extra iter37 audit metadata cannot hide output drift; "
            "nonzero-alpha checks require track-query mutation and unchanged trajectory-query state."
        ),
    }
