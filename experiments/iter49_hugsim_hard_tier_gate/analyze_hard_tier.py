#!/usr/bin/env python3
"""Iteration 49 offline paired analyzer — HUGSIM hard/extreme-tier transfer gate.

Runs ONCE, offline, over the committed proof artifacts (episodes collected from the
box-side iter49_runs root plus receipts.json). Applies EXACTLY the frozen design of
experiments/iter49_hugsim_hard_tier_gate/HYPOTHESIS.md — the iteration-48 analyzer shape
with the registered iteration-49 deltas:

- Scenario list: the lexicographically first 26 of the 36 harder-tier yamls
  (13 scenes x {extreme-00, hard-00}).
- Bootstrap seed 49 (10,000 draws, 26 scenario clusters).
- F1 additionally checks the receipts' monitor_patch_sha byte-identity against the
  committed iteration-48 patch (the byte copy IS the experiment).
- Secondaries add per-episode brake-frame fractions and step-cap episodes (the
  iteration-48 scene-0051 localized over-braking watch) and a descriptive hard-vs-extreme
  split (context only, NOT a bar; no tier claim).

The patch is the iteration-48 byte copy, so the grepped strings keep their I48 names
(SENTINEL_I48_UNION_PATCH_LOADED, SENTINEL_I48_DECISION, sentinel_iter48_decisions.jsonl).
...
SCENES13: list[str] = [
    "scene-0013", "scene-0038", "scene-0041", "scene-0051", "scene-0062",
    "scene-0064", "scene-0071", "scene-0138", "scene-0166", "scene-0167",
    "scene-0254", "scene-0383", "scene-0411",
]
SCENARIOS26: list[str] = [
    f"{s}-{tier}-00" for s in SCENES13 for tier in ("extreme", "hard")
]
RUNS = (1, 2)
ARMS = ("off", "on")
...
# The committed iteration-48 patch, used UNCHANGED as a byte copy (HYPOTHESIS.md).
...
def check_f1(receipts: dict, episodes: list[dict]) -> dict:
    ...
    if receipts.get("monitor_patch_sha") != FROZEN_PATCH_SHA:
        problems.append("monitor-patch-sha-mismatch")
    for e in episodes:
        if e["arm"] != "on":
            continue
...
def firing_statistics(episodes: list[dict]) -> dict:
    ...
    for e in episodes:
        ...
        per_episode.append({
            "episode": f"{e['scenario']}__on_r{e['run']}",
            "steps": e.get("steps"),
            "step_capped": bool(e.get("steps", 0) >= STEP_CAP),
            "brake_frames": ep_brake,
            "monitor_frames": ep_total,
            "brake_fraction": (ep_brake / ep_total) if ep_total else None,
        })
    per_episode.sort(key=lambda r: -(r["brake_fraction"] or 0.0))
    return {
        ...
        "per_episode_brake_fractions": per_episode,
        "step_capped_episodes": [r["episode"] for r in per_episode if r["step_capped"]],
    }
...
def analyze(episodes_root: Path, receipts_path: Path) -> dict:
    ...
    report: dict = {
        "design": {
            ...
            "frozen_patch_sha": FROZEN_PATCH_SHA,
    ...
    # F1 — retuning/byte-identity void (checked before any bar).
    ...
    tier_deltas: dict[str, list[float]] = {"hard": [], "extreme": []}
    for s in SCENARIOS26:
        cluster = []
        tier = "extreme" if "-extreme-" in s else "hard"
        for r in RUNS:
            off = by_key[(s, "off", r)]
            on = by_key[(s, "on", r)]
            d = float(on["hdscore"]) - float(off["hdscore"])
            cluster.append(d)
            tier_deltas[tier].append(d)
            pair_rows.append({
                "scenario": s, "run": r, "tier": tier,
    ...
    report["tier_split_descriptive"] = {
        tier: {
            "mean_delta": statistics.mean(v) if v else None,
            "median_delta": statistics.median(v) if v else None,
            "n_pairs": len(v),
            "note": "context only; 26 pairs/tier is not powered for a tier claim",
        } for tier, v in tier_deltas.items()
    }
    ...
    # F5 — pairing infeasibility, fresh harder-tier OFF-OFF pairs from this run.
    off_off = []
    for s in SCENARIOS26:
...
def render_markdown(report: dict) -> str:
    ...
    for p in report.get("pairs", []):
        lines.append(
            f"| {p['scenario']} | {p['tier']} | {p['run']} | {p['hd_off']:.4f} | "
...
def main() -> int:
    ...
    print(f"iter49 analyzer verdict: {report['verdict']}")
    ...
    if "firing_statistics" in report:
        s = report["firing_statistics"]
        print(f"firing: frames={s['monitor_frames']} fired={s['fired_frames']} "
              f"brake={s['brake_frames']} releases={s['releases']} "
              f"intervention_episodes={s['intervention_episodes']} "
              f"step_capped={s['step_capped_episodes']}")
    return 0
