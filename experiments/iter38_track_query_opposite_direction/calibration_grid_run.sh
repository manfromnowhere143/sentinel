#!/bin/bash
# Iteration-38 calibration grid: prefix-preserving replay for each frozen alpha.
exec > /var/log/sentinel-e38-calibration.log 2>&1
set -x
...
MANIFEST=/tmp/prefix_manifest_calibration_iter38.json
...
run_alpha() {
  ...
  rm -f "$UNIAD/sentinel_e38_calibration_alpha${encoded}.jsonl" \
    "$UNIAD/sentinel_e38_calibration_alpha${encoded}_gt.jsonl" \
  ...
  docker run --name model --rm --gpus all \
    ...
    -e SENTINEL_E38_LOG="/model/sentinel_e38_calibration_alpha${encoded}.jsonl" \
  ...
  echo "waiting for model server iter38 calibration alpha=${alpha}..."
  ...
  cp "$MANIFEST" "$UNIAD/prefix_manifest_calibration_iter38.json"
  docker exec model python -u /model/feeder_intervention_iter38.py \
    --prefix-manifest /model/prefix_manifest_calibration_iter38.json \
    --alpha "$alpha" \
    --port "$port" \
    --out "/model/sentinel_e38_calibration_alpha${encoded}_gt.jsonl"
  docker rm -f model >/dev/null 2>&1 || true
  gzip -kf "$UNIAD/sentinel_e38_calibration_alpha${encoded}.jsonl" \
    "$UNIAD/sentinel_e38_calibration_alpha${encoded}_gt.jsonl"
  echo "E38_CALIBRATION_ALPHA_${encoded}_DONE $(date)"
}
...
run_alpha 0.75 9353
run_alpha 1.00 9354
ls -la "$UNIAD"/sentinel_e38_calibration*.gz
echo "E38_CALIBRATION_DONE $(date)"
