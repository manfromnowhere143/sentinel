#!/usr/bin/env python3
"""Build the iteration-38 opposite track-query direction and offline receipts.

This script is offline-only. It reads committed iteration-29/30/33/36/37
artifacts, derives the fit-only eligible-minus-benign centroid direction over
``sdc_track_query`` alone, and verifies that it is the exact negative of the
committed iteration-37 direction before any GPU replay is eligible.
"""
...
ITER38 = ROOT / "experiments/iter38_track_query_opposite_direction"
OUT = ITER38 / "proof-direction"
...
EXPECTED_ITER37_STATUS = "CALIBRATION_NULL_NO_USABLE_ALPHA"
EXPECTED_TRACK_FEATURE_COUNT = 256
SIGN_EQUIVALENCE_TOLERANCE = 1e-6
...
def derive_direction(
    ...
    direction_std_keep = mu_pos_std - mu_benign_std
    ...
    artifact = {
        "stage": "iter38_track_query_opposite_direction",
        "source": "fit_split_track_query_eligible_lowdiv_centroid_minus_benign_centroid",
...
def sign_equivalence_report(
    iter38_direction_raw: np.ndarray,
    iter37_direction_path: Path,
) -> tuple[dict, list[str]]:
    failures = []
    ...
    if iter37_raw.shape != iter38_direction_raw.shape:
        failures.append(
            f"iter37_direction_shape={iter37_raw.shape} != iter38_direction_shape={iter38_direction_raw.shape}"
        )
        residual = np.asarray([], dtype=np.float64)
    else:
        residual = iter38_direction_raw + iter37_raw
        max_abs = float(np.max(np.abs(residual))) if residual.size else float("inf")
        if max_abs > SIGN_EQUIVALENCE_TOLERANCE:
            failures.append(
                "iter38_direction_not_negative_of_iter37:"
                f"max_abs_sum={max_abs:.9g} > {SIGN_EQUIVALENCE_TOLERANCE:.9g}"
            )

    dot_product = None
    cosine = None
    if iter37_raw.shape == iter38_direction_raw.shape and iter37_raw.size:
        dot_product = float(np.dot(iter38_direction_raw, iter37_raw))
        denom = float(np.linalg.norm(iter38_direction_raw) * np.linalg.norm(iter37_raw))
    ...
    return {
        ...
        "max_abs_direction_sum": (
            float(np.max(np.abs(residual))) if residual.size else None
        ),
        "l2_direction_sum": float(np.linalg.norm(residual)) if residual.size else None,
        "dot_product": dot_product,
        "cosine_similarity": cosine,
        "tolerance": SIGN_EQUIVALENCE_TOLERANCE,
        "sign_equivalence_pass": not failures,
    }, failures
...
def prerequisite_report(args: argparse.Namespace) -> tuple[dict, list[str]]:
    ...
    statuses = {
        ...
        "iter37_result_status": load_status(ITER37 / "RESULT.md"),
    }
    expected = {
        ...
        "iter37_result_status": EXPECTED_ITER37_STATUS,
    }
    ...
    iter37_calibration = json.loads(Path(args.iter37_calibration_report).read_text(encoding="utf-8"))
    if iter37_calibration.get("verdict") != EXPECTED_ITER37_STATUS:
        failures.append(f"iter37_calibration_verdict={iter37_calibration.get('verdict')}")
    if iter37_calibration.get("selected") is not False:
        failures.append(f"iter37_selected={iter37_calibration.get('selected')} != false")
    if iter37_calibration.get("selected_alpha") is not None:
        failures.append(f"iter37_selected_alpha={iter37_calibration.get('selected_alpha')} != null")
    ...
    return {
        **statuses,
        ...
        "iter37_calibration_null": {
            "path": str(Path(args.iter37_calibration_report).relative_to(ROOT)),
            "verdict": iter37_calibration.get("verdict"),
            "selected": iter37_calibration.get("selected"),
            "selected_alpha": iter37_calibration.get("selected_alpha"),
            "prefix_replay_integrity_pass": iter37_calibration.get("prefix_replay_integrity_pass"),
        },
...
def main() -> int:
    ...
    parser.add_argument("--iter37-direction", default=str(ITER37 / "proof-direction/track_query_direction.json"))
    parser.add_argument(
        "--iter37-calibration-report",
        default=str(ITER37 / "proof-calibration/calibration_report.json"),
    )
    ...
    sign_report, sign_failures = sign_equivalence_report(
        arrays["direction_raw"],
        Path(args.iter37_direction),
    )
    direction["iter37_sign_equivalence"] = sign_report
    if sign_failures:
        input_report["s0_failures"].extend(sign_failures)
        write_json(
            out_dir / "direction_input_failure.json",
            {
                "stage": "iter38_track_query_opposite_direction",
    ...
    write_json(out_dir / "track_query_opposite_direction.json", direction)
    report = {
        "stage": "iter38_track_query_opposite_direction",
        "command_line": " ".join(sys.argv),
        **input_report,
        ...
        "claim_boundary": (
            "This artifact prepares the frozen iter38 opposite track-query direction and "
            "prerequisite receipts only. It does not run a model, select an alpha, touch "
            "iteration-12 frames, score a selector, run closed loop, or make a safety claim."
        ),
    }
