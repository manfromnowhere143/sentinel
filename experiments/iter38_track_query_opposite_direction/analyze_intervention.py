#!/usr/bin/env python3
"""Analyze iteration-38 prefix-preserving track-query intervention replay logs.
...
def context_contamination_report(rows: list[dict]) -> dict:
    ...
    for row in context:
        ...
        if row.get("track_query_sha256_changed") is True:
            failures.append(f"{key}:context_track_query_sha256_changed_true")
        if row.get("sdc_traj_query_last_sha256_changed") is True:
            failures.append(f"{key}:context_sdc_traj_query_last_sha256_changed_true")
    return {
...
def analyze_calibration(paths: list[Path]) -> dict:
    ...
    return {
        "stage": "iter38_opposite_direction_calibration_grid",
        ...
        **selection,
        "claim_boundary": (
            "Opposite-direction calibration selects at most one global alpha using target rows "
            "only. It is not a heldout result, selector score, closed-loop result, deployment "
            "result, or safety claim."
        ),
    }
...
def analyze_heldout(paths: list[Path]) -> dict:
    ...
    if prefix_failures:
        ...
        verdict = "TRACK_QUERY_OPPOSITE_DIRECTION_STAGE1_PASS_SUCCESSOR_PREREG_AUTHORIZED"
    return {
        "stage": "iter38_heldout_track_query_opposite_direction",
        "command_line": " ".join(sys.argv),
        "run_summaries": [public_run_summary(summary) for summary in summaries],
        "prefix_replay_integrity_pass": not prefix_failures,
        "prefix_replay_integrity_failures": prefix_failures[:100],
        ...
        **bars,
        "verdict": verdict,
        "claim_boundary": (
            "An opposite-direction pass authorizes only a separate Stage-2 pre-registration. It "
            "does not authorize iteration-12 scoring, selector evaluation, closed-loop work, "
            "deployment, or a safety claim."
        ),
    }
...
def nonzero_track_query_report(summaries: list[dict]) -> dict:
    failures = []
    track_changed = 0
    traj_unchanged = 0
    bridge_changed = 0
    ...
    for summary in summaries:
        for row in summary["target_rows_data"]:
            ...
            if row.get("target_site") != "track_query":
                failures.append(f"{key}:alpha0p50_target_site={row.get('target_site')} != track_query")
            if row.get("track_query_sha256_changed") is not True:
                failures.append(f"{key}:alpha0p50_track_query_sha256_changed_not_true")
            if row.get("original_track_query_sha256") == row.get("intervened_track_query_sha256"):
                failures.append(f"{key}:alpha0p50_original_and_intervened_track_query_sha_equal")
            else:
                track_changed += 1
            if row.get("sdc_traj_query_last_sha256_changed") is not False:
                failures.append(f"{key}:alpha0p50_sdc_traj_query_last_sha256_changed_not_false")
            if row.get("original_sdc_traj_query_last_sha256") != row.get("intervened_sdc_traj_query_last_sha256"):
                failures.append(f"{key}:alpha0p50_sdc_traj_query_last_sha_changed")
            else:
                traj_unchanged += 1
    ...
    if len(unique_keys) != 12:
        failures.append(f"alpha0p50_unique_target_rows_checked={len(unique_keys)} != 12")
    return {
        "alpha0p50_target_rows_checked": checked,
        "alpha0p50_unique_target_rows_checked": len(unique_keys),
        "alpha0p50_changed_track_query_sha_rows": track_changed,
        "alpha0p50_unchanged_sdc_traj_query_last_sha_rows": traj_unchanged,
        "alpha0p50_changed_bridge_sha_rows": bridge_changed,
        "alpha0p50_track_query_site_pass": not failures,
        "alpha0p50_track_query_site_failures": failures[:50],
    }
...
def analyze_canary(
    ...
    site_report = nonzero_track_query_report(model_summaries)
    if not site_report["alpha0p50_track_query_site_pass"]:
        failures.append("alpha0p50_track_query_site_failed")

    return {
        "stage": "iter38_opposite_direction_canary_integrity",
        "command_line": " ".join(sys.argv),
        ...
        **site_report,
        ...
        "claim_boundary": (
            ...
            "compatible target-row projection so extra iter38 audit metadata cannot hide output drift; "
            "nonzero-alpha checks require opposite-direction track-query mutation and unchanged "
            "trajectory-query state."
        ),
    }
