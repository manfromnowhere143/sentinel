#!/usr/bin/env python3
"""Prefix-preserving feeder for iteration-38 track-query intervention replay."""
...
def row_context(scene_name: str, rec: dict, sample_index: int, timestamp_us: int, run_alpha: float) -> dict:
    target_row = bool(rec.get("target_row", False))
    effective_alpha = float(run_alpha) if target_row else 0.0
    return {
        "scene": scene_name,
        ...
        "label_name": rec.get("source_label_name", ""),
        "run_alpha": float(run_alpha),
        "alpha": effective_alpha,
        "intervention_alpha": effective_alpha,
    }


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--prefix-manifest", default="/model/prefix_manifest_iter38.json")
    ...
    ap.add_argument("--out", default="/model/sentinel_e38_gt.jsonl")
    args = ap.parse_args()
    ...
    for sc in ordered_scenes:
        ...
        for sample_index, smp in enumerate(chain):
            ...
            target_row = bool(rec.get("target_row", False))
            context = row_context(sc["name"], rec, sample_index, timestamp_us, args.alpha)
            payload = {
                "images": images,
            ...
            t0 = time.time()
            post(f"{base}/sentinel_e38_context", context)
            ...
            context_frames += int(not target_row)
            scene_frames += 1
    ...
    print(
        "FEEDER_DONE "
        f"run_alpha={args.alpha} rows={done_frames} target_rows={target_frames} "
        f"context_only_rows={context_frames} scenes={len(ordered_scenes)}",
        flush=True,
    )
